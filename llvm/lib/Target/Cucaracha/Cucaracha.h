...
#ifndef LLVM_LIB_TARGET_Cucaracha_Cucaracha_H
#define LLVM_LIB_TARGET_Cucaracha_Cucaracha_H
...
FunctionPass *createCucarachaDelaySlotFillerPass();

void LowerCucarachaMachineInstrToMCInst(const MachineInstr *MI, MCInst &OutMI,
                                        AsmPrinter &AP);
void initializeCucarachaDAGToDAGISelPass(PassRegistry &);
} // namespace llvm

namespace llvm {
// Enums corresponding to Cucaracha condition codes, both icc's and fcc's. These
...
inline static const char *CucarachaCondCodeToString(SPCC::CondCodes CC) {
  switch (CC) {
  case SPCC::ICC_A:
    return "a";
  case SPCC::ICC_N:
