//===-- CucarachaFrameLowering.cpp - Cucaracha Frame Information
//------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the Cucaracha implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
static cl::opt<bool>
    DisableLeafProc("disable-cucaracha-leaf-proc", cl::init(false),
                    cl::desc("Disable Cucaracha leaf procedure optimization."),
                    cl::Hidden);

CucarachaFrameLowering::CucarachaFrameLowering(const CucarachaSubtarget &ST)
    : TargetFrameLowering(TargetFrameLowering::StackGrowsDown,
                          ST.is64Bit() ? Align(16) : Align(8), 0,
                          ST.is64Bit() ? Align(16) : Align(8)) {}

void CucarachaFrameLowering::emitSPAdjustment(MachineFunction &MF,
  ...
  DebugLoc dl;
  const CucarachaInstrInfo &TII = *static_cast<const CucarachaInstrInfo *>(
...
void CucarachaFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  const CucarachaInstrInfo &TII =
      *static_cast<const CucarachaInstrInfo *>(Subtarget.getInstrInfo());
  const CucarachaRegisterInfo &RegInfo =
      *static_cast<const CucarachaRegisterInfo *>(Subtarget.getRegisterInfo());
  ...
  // The Cucaracha ABI is a bit odd in that it requires a reserved 92-byte
  ...
  // Adds the Cucaracha subtarget-specific spill area to the stack
...
bool CucarachaFrameLowering::hasReservedCallFrame(
...
StackOffset CucarachaFrameLowering::getFrameIndexReference(
    const MachineFunction &MF, int FI, Register &FrameReg) const {
  const CucarachaSubtarget &Subtarget = MF.getSubtarget<CucarachaSubtarget>();
  const MachineFrameInfo &MFI = MF.getFrameInfo();
  const CucarachaRegisterInfo *RegInfo = Subtarget.getRegisterInfo();
  const CucarachaMachineFunctionInfo *FuncInfo =
      MF.getInfo<CucarachaMachineFunctionInfo>();
  ...
  // Cucaracha uses FP-based references in general, even when "hasFP" is
...
bool CucarachaFrameLowering::isLeafProc(MachineFunction &MF) const {
...
void CucarachaFrameLowering::remapRegsForLeafProc(MachineFunction &MF) const {
  MachineRegisterInfo &MRI = MF.getRegInfo();
...
void CucarachaFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                                  BitVector &SavedRegs,
                                                  RegScavenger *RS) const {
  TargetFrameLowering::determineCalleeSaves(MF, SavedRegs, RS);
  if (!DisableLeafProc && isLeafProc(MF)) {
    CucarachaMachineFunctionInfo *MFI =
        MF.getInfo<CucarachaMachineFunctionInfo>();
