//===- CucarachaDisassembler.cpp - Disassembler for Cucaracha -----------*- C++
//-*-===//
//
...
//===----------------------------------------------------------------------===//
//
// This file is part of the Cucaracha Disassembler.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "cucaracha-disassembler"

typedef MCDisassembler::DecodeStatus DecodeStatus;

namespace {

/// A disassembler class for Cucaracha.
class CucarachaDisassembler : public MCDisassembler {
public:
  CucarachaDisassembler(const MCSubtargetInfo &STI, MCContext &Ctx)
      : MCDisassembler(STI, Ctx) {}
  virtual ~CucarachaDisassembler() = default;
...
static MCDisassembler *createCucarachaDisassembler(const Target &T,
                                                   const MCSubtargetInfo &STI,
                                                   MCContext &Ctx) {
  return new CucarachaDisassembler(STI, Ctx);
}

extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeCucarachaDisassembler() {
...
// depending on CucarachaRegisterInfo::getPointerRegClass.
...
#include "CucarachaGenDisassemblerTables.inc"
...
DecodeStatus CucarachaDisassembler::getInstruction(MCInst &Instr,
                                                   uint64_t &Size,
  ...
  if (STI.hasFeature(Cucaracha::FeatureV9)) {
    Result = decodeInstruction(DecoderTableCucarachaV932, Instr, Insn, Address,
                               this, STI);
  } else {
    Result = decodeInstruction(DecoderTableCucarachaV832, Instr, Insn, Address,
                               this, STI);
  }
  if (Result != MCDisassembler::Fail)
    return Result;

  Result = decodeInstruction(DecoderTableCucaracha32, Instr, Insn, Address,
                             this, STI);
