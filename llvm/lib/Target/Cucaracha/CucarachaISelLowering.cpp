...
//===----------------------------------------------------------------------===//
//
// This file implements the interfaces that Cucaracha uses to lower LLVM code
...
static bool CC_Cucaracha_Assign_SRet(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
...
static bool CC_Cucaracha_Assign_Split_64(unsigned &ValNo, MVT &ValVT,
                                         MVT &LocVT,
                                         CCValAssign::LocInfo &LocInfo,
                                         ISD::ArgFlagsTy &ArgFlags,
                                         CCState &State) {
...
static bool CC_Cucaracha_Assign_Ret_Split_64(unsigned &ValNo, MVT &ValVT,
                                             MVT &LocVT,
                                             CCValAssign::LocInfo &LocInfo,
                                             ISD::ArgFlagsTy &ArgFlags,
                                             CCState &State) {
...
static bool Analyze_CC_Cucaracha64_Full(bool IsReturn, unsigned &ValNo,
                                        MVT &ValVT, MVT &LocVT,
                                        CCValAssign::LocInfo &LocInfo,
                                        ISD::ArgFlagsTy &ArgFlags,
                                        CCState &State) {
...
static bool Analyze_CC_Cucaracha64_Half(bool IsReturn, unsigned &ValNo,
                                        MVT &ValVT, MVT &LocVT,
                                        CCValAssign::LocInfo &LocInfo,
                                        ISD::ArgFlagsTy &ArgFlags,
                                        CCState &State) {
...
static bool CC_Cucaracha64_Full(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                                CCValAssign::LocInfo &LocInfo,
                                ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_Cucaracha64_Full(false, ValNo, ValVT, LocVT, LocInfo,
                                     ArgFlags, State);
}

static bool CC_Cucaracha64_Half(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                                CCValAssign::LocInfo &LocInfo,
                                ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_Cucaracha64_Half(false, ValNo, ValVT, LocVT, LocInfo,
                                     ArgFlags, State);
}

static bool RetCC_Cucaracha64_Full(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                                   CCValAssign::LocInfo &LocInfo,
                                   ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_Cucaracha64_Full(true, ValNo, ValVT, LocVT, LocInfo,
                                     ArgFlags, State);
}

static bool RetCC_Cucaracha64_Half(unsigned &ValNo, MVT &ValVT, MVT &LocVT,
                                   CCValAssign::LocInfo &LocInfo,
                                   ISD::ArgFlagsTy &ArgFlags, CCState &State) {
  return Analyze_CC_Cucaracha64_Half(true, ValNo, ValVT, LocVT, LocInfo,
                                     ArgFlags, State);
}

#include "CucarachaGenCallingConv.inc"

// The calling conventions in CucarachaCallingConv.td are described in terms of
// the callee's register window. This function translates registers to the
...
bool CucarachaTargetLowering::CanLowerReturn(
  ...
  return CCInfo.CheckReturn(Outs, Subtarget->is64Bit() ? RetCC_Cucaracha64
                                                       : RetCC_Cucaracha32);
}
...
SDValue CucarachaTargetLowering::LowerReturn_32(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs,
    const SmallVectorImpl<SDValue> &OutVals, const SDLoc &DL,
    SelectionDAG &DAG) const {
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_Cucaracha32);
  ...
  if (MF.getFunction().hasStructRetAttr()) {
    CucarachaMachineFunctionInfo *SFI =
        MF.getInfo<CucarachaMachineFunctionInfo>();
...
SDValue CucarachaTargetLowering::LowerReturn_64(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs,
    const SmallVectorImpl<SDValue> &OutVals, const SDLoc &DL,
    SelectionDAG &DAG) const {
  ...
  // Analyze return values.
  CCInfo.AnalyzeReturn(Outs, RetCC_Cucaracha64);
...
SDValue CucarachaTargetLowering::LowerFormalArguments_32(
    SDValue Chain, CallingConv::ID CallConv, bool isVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &dl,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  MachineFunction &MF = DAG.getMachineFunction();
  MachineRegisterInfo &RegInfo = MF.getRegInfo();
  CucarachaMachineFunctionInfo *FuncInfo =
      MF.getInfo<CucarachaMachineFunctionInfo>();
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_Cucaracha32);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i, ++InIdx) {
    ...
    if (Ins[InIdx].Flags.isSRet()) {
      if (InIdx != 0)
        report_fatal_error(
            "cucaracha only supports sret on the first parameter");
      // Get SRet from [%fp+64].
    ...
    if (VA.getValVT() == MVT::i32 || VA.getValVT() == MVT::f32) {
      ...
      report_fatal_error("Cucarachav8 does not handle f128 in calls; "
                         "pass indirectly");
    } else {
...
SDValue CucarachaTargetLowering::LowerFormalArguments_64(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &DL,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  MachineFunction &MF = DAG.getMachineFunction();

  // Analyze arguments according to CC_Cucaracha64.
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_Cucaracha64);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    assert(VA.isMemLoc());
    // The CC_Cucaracha64_Full/Half functions compute stack offsets relative to
    // the beginning of the arguments area at %fp+BIAS+128.
    unsigned Offset = VA.getLocMemOffset() + ArgArea;
    unsigned ValSize = VA.getValVT().getSizeInBits() / 8;
    // Adjust offset for extended arguments, Cucaracha is big-endian.
...
bool CucarachaTargetLowering::IsEligibleForTailCallOptimization(
...
SDValue
CucarachaTargetLowering::LowerCall_32(TargetLowering::CallLoweringInfo &CLI,
                                      SmallVectorImpl<SDValue> &InVals) const {
  ...
  CCState CCInfo(CallConv, isVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(Outs, CC_Cucaracha32);
  ...
  const CucarachaRegisterInfo *TRI = Subtarget->getRegisterInfo();
  const uint32_t *Mask =
      ((hasReturnsTwice)
  ...
  RVInfo.AnalyzeCallResult(Ins, RetCC_Cucaracha32);
...
Register
CucarachaTargetLowering::getRegisterByName(const char *RegName, LLT VT,
...
//
// The Cucaracha v9 ABI requires that floating point arguments are treated the
// same as integers when calling a varargs function. This does not apply to the
...
SDValue
CucarachaTargetLowering::LowerCall_64(TargetLowering::CallLoweringInfo &CLI,
                                      SmallVectorImpl<SDValue> &InVals) const {
  SelectionDAG &DAG = CLI.DAG;
  SDLoc DL = CLI.DL;
  SDValue Chain = CLI.Chain;
  ...
  CCState CCInfo(CLI.CallConv, CLI.IsVarArg, DAG.getMachineFunction(), ArgLocs,
                 *DAG.getContext());
  CCInfo.AnalyzeCallOperands(CLI.Outs, CC_Cucaracha64);

  CLI.IsTailCall = CLI.IsTailCall && IsEligibleForTailCallOptimization(
                                         CCInfo, CLI, DAG.getMachineFunction());

  // Get the size of the outgoing arguments stack space requirement.
  // The stack offset computed by CC_Cucaracha64 includes all arguments.
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    case CCValAssign::BCvt:
      ...
      // Cucaracha does not support i128 natively. Lower it into two i64, see
      // below.
  ...
  RVInfo.AnalyzeCallResult(CLI.Ins, RetCC_Cucaracha64);
...
CucarachaTargetLowering::shouldExpandAtomicRMWInIR(AtomicRMWInst *AI) const {
...
/// intCondCCodeToRcond - Convert a DAG integer condition code to a Cucaracha
/// rcond condition.
...
/// IntCondCCodeToICC - Convert a DAG integer condition code to a Cucaracha ICC
/// condition.
...
/// FPCondCCodeToFCC - Convert a DAG floatingp oint condition code to a
/// Cucaracha FCC condition.
...
CucarachaTargetLowering::CucarachaTargetLowering(const TargetMachine &TM,
                                                 const CucarachaSubtarget &STI)
  ...
  if (Subtarget->is64Bit()) {
    ...
    // On 32bit cucaracha, we define a double-register 32bit register
  ...
  // Cucaracha doesn't have i1 sign extending load
  ...
  // Cucaracha doesn't have sext_inreg, replace them with shl/sra
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i16, Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i8, Expand);
  setOperationAction(ISD::SIGN_EXTEND_INREG, MVT::i1, Expand);

  // Cucaracha has no REM or DIVREM operations.
  setOperationAction(ISD::UREM, MVT::i32, Expand);
  setOperationAction(ISD::SREM, MVT::i32, Expand);
  setOperationAction(ISD::SDIVREM, MVT::i32, Expand);
  setOperationAction(ISD::UDIVREM, MVT::i32, Expand);

  // ... nor does CucarachaV9.
  if (Subtarget->is64Bit()) {
  ...
  // Cucaracha has no select or setcc: expand to SELECT_CC.
  ...
  // Cucaracha doesn't have BRCOND either, it has BR_CC.
  ...
  // ATOMICs.
  // Atomics are supported on CucarachaV9. 32-bit atomics are also
  // supported by some Leon CucarachaV8 variants. Otherwise, atomics
  // are unsupported.
  if (Subtarget->isV9())
  ...
  if (!Subtarget->isV9()) {
    // CucarachaV8 does not have FNEGD and FABSD.
...
bool CucarachaTargetLowering::useSoftFloat() const {
...
EVT CucarachaTargetLowering::getSetCCResultType(const DataLayout &,
...
/// combiner.
void CucarachaTargetLowering::computeKnownBitsForTargetNode(
...
SDValue CucarachaTargetLowering::withTargetFlags(SDValue Op, unsigned TF,
                                                 SelectionDAG &DAG) const {
...
SDValue CucarachaTargetLowering::makeHiLoPair(SDValue Op, unsigned HiTF,
                                              unsigned LoTF,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue CucarachaTargetLowering::makeAddress(SDValue Op,
                                             SelectionDAG &DAG) const {
  SDLoc DL(Op);
  EVT VT = getPointerTy(DAG.getDataLayout());

  // Handle PIC mode first. Cucaracha needs a got load for every variable!
...
SDValue CucarachaTargetLowering::LowerConstantPool(SDValue Op,
                                                   SelectionDAG &DAG) const {
  return makeAddress(Op, DAG);
}

SDValue CucarachaTargetLowering::LowerBlockAddress(SDValue Op,
                                                   SelectionDAG &DAG) const {
  return makeAddress(Op, DAG);
}

SDValue
CucarachaTargetLowering::LowerGlobalTLSAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
...
SDValue CucarachaTargetLowering::LowerF128_LibCallArg(SDValue Chain,
                                                      ArgListTy &Args,
                                                      SDValue Arg,
                                                      const SDLoc &DL,
                                                      SelectionDAG &DAG) const {
...
SDValue CucarachaTargetLowering::LowerF128Op(SDValue Op, SelectionDAG &DAG,
...
SDValue CucarachaTargetLowering::LowerF128Compare(SDValue LHS, SDValue RHS,
                                                  unsigned &SPCC,
                                                  const SDLoc &DL,
                                                  SelectionDAG &DAG) const {
...
static SDValue LowerF128_FPEXTEND(SDValue Op, SelectionDAG &DAG,
                                  const CucarachaTargetLowering &TLI) {
...
static SDValue LowerBR_CC(SDValue Op, SelectionDAG &DAG,
                          const CucarachaTargetLowering &TLI, bool hasHardQuad,
...
static SDValue LowerSELECT_CC(SDValue Op, SelectionDAG &DAG,
                              const CucarachaTargetLowering &TLI,
                              bool hasHardQuad, bool isV9, bool is64Bit) {
...
static SDValue LowerDYNAMIC_STACKALLOC(SDValue Op, SelectionDAG &DAG,
  ...
  if (Subtarget->is64Bit()) {
    ...
    // On Cucaracha32, the size of the spill area is 92. Unfortunately,
...
// Custom lower UMULO/SMULO for Cucaracha. This code is similar to ExpandNode()
...
SDValue
CucarachaTargetLowering::LowerINTRINSIC_WO_CHAIN(SDValue Op,
                                                 SelectionDAG &DAG) const {
...
SDValue CucarachaTargetLowering::bitcastConstantFPToInt(
    ConstantFPSDNode *C, const SDLoc &DL, SelectionDAG &DAG) const {
...
SDValue
CucarachaTargetLowering::PerformBITCASTCombine(SDNode *N,
...
SDValue CucarachaTargetLowering::PerformDAGCombine(SDNode *N,
                                                   DAGCombinerInfo &DCI) const {
  switch (N->getOpcode()) {
  default:
    break;
...
MachineBasicBlock *CucarachaTargetLowering::EmitInstrWithCustomInserter(
...
MachineBasicBlock *
CucarachaTargetLowering::expandSelectCC(MachineInstr &MI, MachineBasicBlock *BB,
...
//===----------------------------------------------------------------------===//
//                         Cucaracha Inline Assembly Support
//===----------------------------------------------------------------------===//

/// getConstraintType - Given a constraint letter, return the type of
/// constraint it is for this target.
CucarachaTargetLowering::ConstraintType
CucarachaTargetLowering::getConstraintType(StringRef Constraint) const {
...
CucarachaTargetLowering::getSingleConstraintMatchWeight(
...
void CucarachaTargetLowering::LowerAsmOperandForConstraint(
...
CucarachaTargetLowering::getRegForInlineAsmConstraint(
...
bool CucarachaTargetLowering::isOffsetFoldingLegal(
    const GlobalAddressSDNode *GA) const {
  // The Cucaracha target isn't yet aware of offsets.
  return false;
}

void CucarachaTargetLowering::ReplaceNodeResults(
...
bool CucarachaTargetLowering::useLoadStackGuardNode() const {
...
void CucarachaTargetLowering::insertSSPDeclarations(Module &M) const {
  if (!Subtarget->isTargetLinux())
