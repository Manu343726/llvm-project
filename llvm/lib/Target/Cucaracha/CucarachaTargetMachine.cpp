...
bool CucarachaPassConfig::addPreISel() { return false; }

bool CucarachaPassConfig::addInstSelector() {
  addPass(createCucarachaISelDag(getCucarachaTargetMachine(), getOptLevel()));
  return false;
}
...
extern "C" void LLVMInitializeCucarachaTarget() {
  RegisterTargetMachine<CucarachaTargetMachine> X(
      llvm::getTheCucarachaTarget());
}
