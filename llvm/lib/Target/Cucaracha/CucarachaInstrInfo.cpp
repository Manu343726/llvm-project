...
static cl::opt<unsigned> BPccDisplacementBits(
    "cucaracha-bpcc-offset-bits", cl::Hidden, cl::init(19),
    cl::desc("Restrict range of BPcc/FBPfcc instructions (DEBUG)"));

static cl::opt<unsigned>
    BPrDisplacementBits("cucaracha-bpr-offset-bits", cl::Hidden, cl::init(16),
                        cl::desc("Restrict range of BPr instructions (DEBUG)"));

// Pin the vtable to this file.
void CucarachaInstrInfo::anchor() {}

CucarachaInstrInfo::CucarachaInstrInfo(CucarachaSubtarget &ST)
    : CucarachaGenInstrInfo(SP::ADJCALLSTACKDOWN, SP::ADJCALLSTACKUP), RI(),
      Subtarget(ST) {}
...
static void parseCondBranch(MachineInstr *LastInst, MachineBasicBlock *&Target,
  ...
  // it can use the information to emit the correct Cucaracha branch opcode.
...
MachineBasicBlock *
CucarachaInstrInfo::getBranchDestBlock(const MachineInstr &MI) const {
  switch (MI.getOpcode()) {
  default:
...
unsigned CucarachaInstrInfo::insertBranch(
  ...
  assert((Cond.size() <= 3) &&
         "Cucaracha branch conditions should have at most three components!");
...
bool CucarachaInstrInfo::isBranchOffsetInRange(unsigned BranchOpc,
...
Register CucarachaInstrInfo::getGlobalBaseReg(MachineFunction *MF) const {
  CucarachaMachineFunctionInfo *CucarachaFI =
      MF->getInfo<CucarachaMachineFunctionInfo>();
  Register GlobalBaseReg = CucarachaFI->getGlobalBaseReg();
  if (GlobalBaseReg)
  ...
  CucarachaFI->setGlobalBaseReg(GlobalBaseReg);
  return GlobalBaseReg;
}

unsigned CucarachaInstrInfo::getInstSizeInBytes(const MachineInstr &MI) const {
...
bool CucarachaInstrInfo::expandPostRAPseudo(MachineInstr &MI) const {
  ...
  case TargetOpcode::LOAD_STACK_GUARD: {
    ...
    // offsetof(tcbhead_t, stack_guard) from sysdeps/cucaracha/nptl/tls.h in
    // glibc.
