...
const char *LEGTargetLowering::getTargetNodeName(unsigned Opcode) const {
  ...
  default:
    return NULL;
  case LEGISD::RET_FLAG:
    return "RetFlag";
  case LEGISD::LOAD_SYM:
    return "LOAD_SYM";
  case LEGISD::MOVEi32:
    return "MOVEi32";
  case LEGISD::CALL:
...
SDValue LEGTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  const unsigned ArgsSize =
      alignTo(CCInfo.getStackSize(), LEGFrameLowering::stackAlign());
...
SDValue LEGTargetLowering::LowerFormalArguments(
  ...
  for (auto &VA : ArgLocs) {
    if (VA.isRegLoc()) {
      ...
      const Register VReg = RegInfo.createVirtualRegister(&LEG::GRRegsRegClass);
