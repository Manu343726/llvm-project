...
void LEGDAGToDAGISel::SelectMoveImmediate(SDNode *N) {
  ...
  SDNode *Move = CurDAG->SelectNodeTo(N, LEG::MOVLOi16, MVT::i32, ConstLo);

  // Select the low part of the immediate move, if needed.
  if (ImmHi) {
    SDValue ConstHi = CurDAG->getTargetConstant(ImmHi >> 16, N, MVT::i32);
    CurDAG->SelectNodeTo(N, LEG::MOVHIi16, MVT::i32, SDValue(Move, 0), ConstHi);
  }
}

void LEGDAGToDAGISel::SelectConditionalBranch(SDNode *N) {
