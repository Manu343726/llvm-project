//===-- FantasyInstrInfo.cpp - Fantasy Instruction Information ------*- C++
//-*-===//
//
...
FantasyInstrInfo::FantasyInstrInfo()
    : FantasyGenInstrInfo(Fantasy::ADJCALLSTACKDOWN, Fantasy::ADJCALLSTACKUP),
      RegisterInfo() {}

void FantasyInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
                                   MachineBasicBlock::iterator Position,
                                   const DebugLoc &DL,
                                   MCRegister DestinationRegister,
                                   MCRegister SourceRegister,
                                   bool KillSource) const {
  if (!Fantasy::GPRRegClass.contains(DestinationRegister, SourceRegister)) {
    llvm_unreachable("Impossible reg-to-reg copy");
  }

  BuildMI(MBB, Position, DL, get(Fantasy::OR_I_LO), DestinationRegister)
      .addReg(SourceRegister, getKillRegState(KillSource))
      .addImm(0);
}

void FantasyInstrInfo::storeRegToStackSlot(
  ...
  if (!Fantasy::GPRRegClass.hasSubClassEq(RegisterClass)) {
    llvm_unreachable("Can't store this register to stack slot");
  }
  BuildMI(MBB, Position, DL, get(Fantasy::SW_RI))
...
void FantasyInstrInfo::loadRegFromStackSlot(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator Position,
  ...
  BuildMI(MBB, Position, DL, get(Fantasy::LDW_RI), DestinationRegister)
      .addFrameIndex(FrameIndex)
      .addImm(0)
      .addImm(LPAC::ADD);
}

bool FantasyInstrInfo::areMemAccessesTriviallyDisjoint(
...
bool FantasyInstrInfo::expandPostRAPseudo(MachineInstr & /*MI*/) const {
  return false;
}
...
FantasyInstrInfo::decomposeMachineOperandsTargetFlags(unsigned TF) const {
  return std::make_pair(TF, 0u);
}

ArrayRef<std::pair<unsigned, const char *>>
FantasyInstrInfo::getSerializableDirectMachineOperandTargetFlags() const {
  using namespace FantasyII;
...
bool FantasyInstrInfo::analyzeCompare(const MachineInstr &MI, unsigned &SrcReg,
                                      unsigned &SrcReg2, int &CmpMask,
                                      int &CmpValue) const {
  switch (MI.getOpcode()) {
  default:
    break;
  case Fantasy::SFSUB_F_RI_LO:
  case Fantasy::SFSUB_F_RI_HI:
    SrcReg = MI.getOperand(0).getReg();
    SrcReg2 = 0;
    CmpMask = ~0;
    CmpValue = MI.getOperand(1).getImm();
    return true;
  case Fantasy::SFSUB_F_RR:
    SrcReg = MI.getOperand(0).getReg();
...
inline static bool isRedundantFlagInstr(MachineInstr *CmpI, unsigned SrcReg,
                                        ...
                                        MachineInstr *OI) {
  if (CmpI->getOpcode() == Fantasy::SFSUB_F_RR &&
      OI->getOpcode() == Fantasy::SUB_R &&
  ...
  if (((CmpI->getOpcode() == Fantasy::SFSUB_F_RI_LO &&
        OI->getOpcode() == Fantasy::SUB_I_LO) ||
       (CmpI->getOpcode() == Fantasy::SFSUB_F_RI_HI &&
        OI->getOpcode() == Fantasy::SUB_I_HI)) &&
      OI->getOperand(1).getReg() == SrcReg &&
...
inline static unsigned flagSettingOpcodeVariant(unsigned OldOpcode) {
  switch (OldOpcode) {
  case Fantasy::ADD_I_HI:
    return Fantasy::ADD_F_I_HI;
  case Fantasy::ADD_I_LO:
    return Fantasy::ADD_F_I_LO;
  case Fantasy::ADD_R:
    return Fantasy::ADD_F_R;
  case Fantasy::ADDC_I_HI:
    return Fantasy::ADDC_F_I_HI;
  case Fantasy::ADDC_I_LO:
    return Fantasy::ADDC_F_I_LO;
  case Fantasy::ADDC_R:
    return Fantasy::ADDC_F_R;
  case Fantasy::AND_I_HI:
    return Fantasy::AND_F_I_HI;
  case Fantasy::AND_I_LO:
    return Fantasy::AND_F_I_LO;
  case Fantasy::AND_R:
    return Fantasy::AND_F_R;
  case Fantasy::OR_I_HI:
    return Fantasy::OR_F_I_HI;
  case Fantasy::OR_I_LO:
    return Fantasy::OR_F_I_LO;
  case Fantasy::OR_R:
    return Fantasy::OR_F_R;
  case Fantasy::SL_I:
    return Fantasy::SL_F_I;
  case Fantasy::SRL_R:
    return Fantasy::SRL_F_R;
  case Fantasy::SA_I:
    return Fantasy::SA_F_I;
  case Fantasy::SRA_R:
    return Fantasy::SRA_F_R;
  case Fantasy::SUB_I_HI:
    return Fantasy::SUB_F_I_HI;
  case Fantasy::SUB_I_LO:
    return Fantasy::SUB_F_I_LO;
  case Fantasy::SUB_R:
    return Fantasy::SUB_F_R;
  case Fantasy::SUBB_I_HI:
    return Fantasy::SUBB_F_I_HI;
  case Fantasy::SUBB_I_LO:
    return Fantasy::SUBB_F_I_LO;
  case Fantasy::SUBB_R:
    return Fantasy::SUBB_F_R;
  case Fantasy::XOR_I_HI:
    return Fantasy::XOR_F_I_HI;
  case Fantasy::XOR_I_LO:
    return Fantasy::XOR_F_I_LO;
  case Fantasy::XOR_R:
    return Fantasy::XOR_F_R;
  default:
    return Fantasy::NOP;
  }
}

bool FantasyInstrInfo::optimizeCompareInstr(
  ...
  if (SrcReg2 != 0)
    ...
    if (CmpInstr.getOpcode() == Fantasy::SFSUB_F_RI_LO)
      MI = nullptr;
    else
      return false;
  }
  ...
  for (--I; I != E; --I) {
    ...
    if (Instr.modifiesRegister(Fantasy::SR, TRI) ||
        Instr.readsRegister(Fantasy::SR, TRI))
  ...
  if (flagSettingOpcodeVariant(MI->getOpcode()) != Fantasy::NOP) {
    bool isSafe = false;
    ...
    while (!isSafe && ++I != E) {
      ...
      for (unsigned IO = 0, EO = Instr.getNumOperands(); !isSafe && IO != EO;
        ...
        if (MO.isRegMask() && MO.clobbersPhysReg(Fantasy::SR)) {
          isSafe = true;
          break;
        }
        if (!MO.isReg() || MO.getReg() != Fantasy::SR)
          continue;
        if (MO.isDef()) {
          isSafe = true;
          break;
...
bool FantasyInstrInfo::analyzeSelect(const MachineInstr &MI,
                                     SmallVectorImpl<MachineOperand> &Cond,
                                     unsigned &TrueOp, unsigned &FalseOp,
                                     bool &Optimizable) const {
  assert(MI.getOpcode() == Fantasy::SELECT && "unknown select instruction");
  // Select operands:
  // 0: Def.
  // 1: True use.
  // 2: False use.
...
MachineInstr *
FantasyInstrInfo::optimizeSelect(MachineInstr &MI,
...
bool FantasyInstrInfo::analyzeBranch(MachineBasicBlock &MBB,
  ...
  while (Instruction != MBB.begin()) {
    ...
    if (Instruction->getOpcode() == Fantasy::BT) {
      if (!AllowModify) {
...
// reversed.
bool FantasyInstrInfo::reverseBranchCondition(
    SmallVectorImpl<llvm::MachineOperand> &Condition) const {
  assert((Condition.size() == 1) &&
         "Fantasy branch conditions should have one component.");
...
unsigned FantasyInstrInfo::insertBranch(MachineBasicBlock &MBB,
                                        MachineBasicBlock *TrueBlock,
                                        MachineBasicBlock *FalseBlock,
  ...
  if (Condition.empty()) {
    ...
    BuildMI(&MBB, DL, get(Fantasy::BT)).addMBB(TrueBlock);
    return 1;
  }
  ...
  BuildMI(&MBB, DL, get(Fantasy::BRCC))
      .addMBB(TrueBlock)
  ...
  BuildMI(&MBB, DL, get(Fantasy::BT)).addMBB(FalseBlock);
  return 2;
}

unsigned FantasyInstrInfo::removeBranch(MachineBasicBlock &MBB,
  ...
  while (Instruction != MBB.begin()) {
    ...
    if (Instruction->isDebugInstr())
      continue;
    if (Instruction->getOpcode() != Fantasy::BT &&
        Instruction->getOpcode() != Fantasy::BRCC) {
      break;
    }
...
unsigned FantasyInstrInfo::isLoadFromStackSlot(const MachineInstr &MI,
                                               int &FrameIndex) const {
  if (MI.getOpcode() == Fantasy::LDW_RI)
...
unsigned FantasyInstrInfo::isLoadFromStackSlotPostFE(const MachineInstr &MI,
                                                     int &FrameIndex) const {
  if (MI.getOpcode() == Fantasy::LDW_RI) {
    unsigned Reg;
...
unsigned FantasyInstrInfo::isStoreToStackSlot(const MachineInstr &MI,
                                              int &FrameIndex) const {
  if (MI.getOpcode() == Fantasy::SW_RI)
...
bool FantasyInstrInfo::getMemOperandWithOffsetWidth(
...
bool FantasyInstrInfo::getMemOperandWithOffset(
    const MachineInstr &LdSt, const MachineOperand *&BaseOp, int64_t &Offset,
