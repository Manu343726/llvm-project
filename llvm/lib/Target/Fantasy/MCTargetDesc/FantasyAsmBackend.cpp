//===-- FantasyAsmBackend.cpp - Fantasy Assembler Backend
//---------------------===//
//
...
namespace {
class FantasyAsmBackend : public MCAsmBackend {
  Triple::OSType OSType;

public:
  FantasyAsmBackend(const Target &T, Triple::OSType OST)
  ...
  unsigned getNumFixupKinds() const override {
    return Fantasy::NumTargetFixupKinds;
  }
...
bool FantasyAsmBackend::writeNopData(raw_ostream &OS, uint64_t Count) const {
  if ((Count % 4) != 0)
    return false;
...
void FantasyAsmBackend::applyFixup(const MCAssembler &Asm, const MCFixup &Fixup,
...
std::unique_ptr<MCObjectTargetWriter>
FantasyAsmBackend::createObjectTargetWriter() const {
  return createFantasyELFObjectWriter(
      MCELFObjectTargetWriter::getOSABI(OSType));
}

const MCFixupKindInfo &
FantasyAsmBackend::getFixupKindInfo(MCFixupKind Kind) const {
  static const MCFixupKindInfo Infos[Fantasy::NumTargetFixupKinds] = {
...
MCAsmBackend *
llvm::createFantasyAsmBackend(const Target &T, const MCSubtargetInfo &STI,
  ...
  return new FantasyAsmBackend(T, TT.getOS());
}
