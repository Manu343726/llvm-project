//===-- FantasyDelaySlotFiller.cpp - Fantasy delay slot filler
//----------------===//
//
...
static cl::opt<bool>
    NopDelaySlotFiller("lanai-nop-delay-filler", cl::init(false),
                       cl::desc("Fill Fantasy delay slots with NOPs."),
                       cl::Hidden);
...
struct Filler : public MachineFunctionPass {
  ...
  StringRef getPassName() const override { return "Fantasy Delay Slot Filler"; }

  bool runOnMachineBasicBlock(MachineBasicBlock &MBB);

  bool runOnMachineFunction(MachineFunction &MF) override {
    const FantasySubtarget &Subtarget = MF.getSubtarget<FantasySubtarget>();
...
// createFantasyDelaySlotFillerPass - Returns a pass that fills in delay
// slots in Fantasy MachineFunctions
FunctionPass *
llvm::createFantasyDelaySlotFillerPass(const FantasyTargetMachine & /*tm*/) {
  return new Filler();
}
  ...
  for (MachineBasicBlock::instr_iterator I = MBB.instr_begin();
    ...
    if (I->getDesc().hasDelaySlot()) {
      ...
      if (I->getOpcode() == Fantasy::RET) {
        ...
        assert(RI->getOpcode() == Fantasy::LDW_RI &&
               RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == Fantasy::FP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == Fantasy::FP &&
               RI->getOperand(2).isImm() && RI->getOperand(2).getImm() == -8);
        ++RI;
        assert(RI->getOpcode() == Fantasy::ADD_I_LO &&
               RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == Fantasy::SP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == Fantasy::FP);
        ...
        if (!NopDelaySlotFiller && findDelayInstr(MBB, I, J)) {
          ...
          BuildMI(MBB, std::next(I), DebugLoc(), TII->get(Fantasy::NOP));
        }
        ++FilledSlots;
      }
