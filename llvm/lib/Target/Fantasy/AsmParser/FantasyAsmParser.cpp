//===-- FantasyAsmParser.cpp - Parse Fantasy assembly to MCInst instructions
//--===//
//
...
struct FantasyOperand;

class FantasyAsmParser : public MCTargetAsmParser {
  // Parse operands
  std::unique_ptr<FantasyOperand> parseRegister();

  std::unique_ptr<FantasyOperand> parseImmediate();

  std::unique_ptr<FantasyOperand> parseIdentifier();
...
#include "FantasyGenAsmMatcher.inc"
...
public:
  FantasyAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
...
// FantasyOperand - Instances of this class represented a parsed machine
// instruction
struct FantasyOperand : public MCParsedAsmOperand {
  enum KindTy {
    TOKEN,
    REGISTER,
    IMMEDIATE,
  ...
  explicit FantasyOperand(KindTy Kind) : MCParsedAsmOperand(), Kind(Kind) {}
...
public:
  ...
  bool isHiImm16() {
    ...
    if (const FantasyMCExpr *SymbolRefExpr = dyn_cast<FantasyMCExpr>(Imm.Value))
      return SymbolRefExpr->getKind() == FantasyMCExpr::VK_Fantasy_ABS_HI;

    // Binary expression
    if (const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(Imm.Value))
      if (const FantasyMCExpr *SymbolRefExpr =
              dyn_cast<FantasyMCExpr>(BinaryExpr->getLHS()))
        return SymbolRefExpr->getKind() == FantasyMCExpr::VK_Fantasy_ABS_HI;
  ...
  bool isLoImm16() {
    ...
    // Symbolic reference expression
    if (const FantasyMCExpr *SymbolRefExpr = dyn_cast<FantasyMCExpr>(Imm.Value))
      return SymbolRefExpr->getKind() == FantasyMCExpr::VK_Fantasy_ABS_LO;
  ...
  bool isLoImm21() {
    ...
    // Symbolic reference expression
    if (const FantasyMCExpr *SymbolRefExpr = dyn_cast<FantasyMCExpr>(Imm.Value))
      return SymbolRefExpr->getKind() == FantasyMCExpr::VK_Fantasy_None;
  ...
  void addLoImm16Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      Inst.addOperand(
          MCOperand::createImm(static_cast<int32_t>(ConstExpr->getValue())));
    else if (isa<FantasyMCExpr>(getImm())) {
#ifndef NDEBUG
      const FantasyMCExpr *SymbolRefExpr = dyn_cast<FantasyMCExpr>(getImm());
      assert(SymbolRefExpr &&
             SymbolRefExpr->getKind() == FantasyMCExpr::VK_Fantasy_ABS_LO);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else if (isa<MCBinaryExpr>(getImm())) {
#ifndef NDEBUG
      const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(getImm());
      assert(BinaryExpr && isa<FantasyMCExpr>(BinaryExpr->getLHS()) &&
             cast<FantasyMCExpr>(BinaryExpr->getLHS())->getKind() ==
                 FantasyMCExpr::VK_Fantasy_ABS_LO);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else
  ...
  void addHiImm16Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      ...
      const FantasyMCExpr *SymbolRefExpr = dyn_cast<FantasyMCExpr>(getImm());
      assert(SymbolRefExpr &&
             SymbolRefExpr->getKind() == FantasyMCExpr::VK_Fantasy_ABS_HI);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else if (isa<MCBinaryExpr>(getImm())) {
#ifndef NDEBUG
      ...
      assert(BinaryExpr && isa<FantasyMCExpr>(BinaryExpr->getLHS()) &&
             cast<FantasyMCExpr>(BinaryExpr->getLHS())->getKind() ==
                 FantasyMCExpr::VK_Fantasy_ABS_HI);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else
      assert(false && "Operand type not supported.");
  ...
  void addLoImm21Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      ...
      const FantasyMCExpr *SymbolRefExpr = dyn_cast<FantasyMCExpr>(getImm());
      assert(SymbolRefExpr &&
             SymbolRefExpr->getKind() == FantasyMCExpr::VK_Fantasy_None);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
  ...
  static std::unique_ptr<FantasyOperand> CreateToken(StringRef Str,
                                                     SMLoc Start) {
    auto Op = std::make_unique<FantasyOperand>(TOKEN);
  ...
  static std::unique_ptr<FantasyOperand> createReg(unsigned RegNum, SMLoc Start,
                                                   SMLoc End) {
    auto Op = std::make_unique<FantasyOperand>(REGISTER);
  ...
  static std::unique_ptr<FantasyOperand> createImm(const MCExpr *Value,
                                                   SMLoc Start, SMLoc End) {
    auto Op = std::make_unique<FantasyOperand>(IMMEDIATE);
    Op->Imm.Value = Value;
    Op->StartLoc = Start;
    Op->EndLoc = End;
    return Op;
  ...
  static std::unique_ptr<FantasyOperand>
  MorphToMemImm(std::unique_ptr<FantasyOperand> Op) {
  ...
  static std::unique_ptr<FantasyOperand>
  MorphToMemRegReg(unsigned BaseReg, std::unique_ptr<FantasyOperand> Op,
                   unsigned AluOp) {
  ...
  static std::unique_ptr<FantasyOperand>
  MorphToMemRegImm(unsigned BaseReg, std::unique_ptr<FantasyOperand> Op,
                   unsigned AluOp) {
    const MCExpr *Imm = Op->getImm();
...
bool FantasyAsmParser::ParseDirective(AsmToken /*DirectiveId*/) { return true; }

bool FantasyAsmParser::MatchAndEmitInstruction(SMLoc IdLoc, unsigned &Opcode,
                                               OperandVector &Operands,
                                               MCStreamer &Out,
                                               uint64_t &ErrorInfo,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      ErrorLoc = ((FantasyOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IdLoc;
    }
...
std::unique_ptr<FantasyOperand> FantasyAsmParser::parseRegister() {
  ...
  if (Lexer.getKind() == AsmToken::Identifier) {
    ...
    return FantasyOperand::createReg(RegNum, Start, End);
  }
  return nullptr;
}

bool FantasyAsmParser::ParseRegister(unsigned &RegNum, SMLoc &StartLoc,
                                     SMLoc &EndLoc) {
  const AsmToken &Tok = getParser().getTok();
  StartLoc = Tok.getLoc();
  EndLoc = Tok.getEndLoc();
  std::unique_ptr<FantasyOperand> Op = parseRegister();
  if (Op != nullptr)
    RegNum = Op->getReg();
  return (Op == nullptr);
}

std::unique_ptr<FantasyOperand> FantasyAsmParser::parseIdentifier() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  const MCExpr *Res, *RHS = nullptr;
  FantasyMCExpr::VariantKind Kind = FantasyMCExpr::VK_Fantasy_None;
  ...
  // identifier.
  if (Kind != FantasyMCExpr::VK_Fantasy_None) {
  ...
  Res = FantasyMCExpr::create(Kind, Expr, getContext());

  // Nest if this was an addition
  if (RHS)
    Res = MCBinaryExpr::createAdd(Res, RHS, getContext());

  return FantasyOperand::createImm(Res, Start, End);
}

std::unique_ptr<FantasyOperand> FantasyAsmParser::parseImmediate() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  ...
  case AsmToken::Dot:
    if (!Parser.parseExpression(ExprVal))
      return FantasyOperand::createImm(ExprVal, Start, End);
    LLVM_FALLTHROUGH;
  default:
    return nullptr;
  }
...
unsigned FantasyAsmParser::parseAluOperator(bool PreOp, bool PostOp) {
  StringRef IdString;
  Parser.parseIdentifier(IdString);
  unsigned AluCode = LPAC::stringToFantasyAluCode(IdString);
...
bool FantasyAsmParser::parsePrePost(StringRef Type, int *OffsetValue) {
...
bool shouldBeSls(const FantasyOperand &Op) {
  ...
  if (const FantasyMCExpr *SymbolRefExpr = dyn_cast<FantasyMCExpr>(Op.getImm()))
    return SymbolRefExpr->getKind() == FantasyMCExpr::VK_Fantasy_None;
  ...
  if (const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(Op.getImm())) {
    const FantasyMCExpr *LHSSymbolRefExpr =
        dyn_cast<FantasyMCExpr>(BinaryExpr->getLHS());
    return (LHSSymbolRefExpr &&
            LHSSymbolRefExpr->getKind() == FantasyMCExpr::VK_Fantasy_None);
  }
  return false;
}
...
FantasyAsmParser::parseMemoryOperand(OperandVector &Operands) {
  ...
  if (Operands[0]->isToken())
    Type = static_cast<FantasyOperand *>(Operands[0].get())->getToken();
  ...
  Parser.Lex(); // Eat the '['.
  std::unique_ptr<FantasyOperand> Offset = nullptr;
  if (Op)
    Offset.swap(Op);
  ...
  if (!Op) {
    if (!Offset) {
      if ((Op = parseImmediate()) && Lexer.is(AsmToken::RBrac)) {
        ...
        // SLS, the rest as RM.
        if (shouldBeSls(*Op)) {
          Operands.push_back(FantasyOperand::MorphToMemImm(std::move(Op)));
        } else {
          ...
          Operands.push_back(FantasyOperand::MorphToMemRegImm(
              Fantasy::R0, std::move(Op), LPAC::ADD));
        }
        return MatchOperand_Success;
      }
    }
  ...
  if (Lexer.is(AsmToken::RBrac)) {
    ...
    if (!Offset) {
      ...
      Offset = FantasyOperand::createImm(OffsetConstExpr, Start, End);
    }
  } else {
  ...
  Operands.push_back(Offset->isImm() ? FantasyOperand::MorphToMemRegImm(
                                           BaseReg, std::move(Offset), AluOp)
                                     : FantasyOperand::MorphToMemRegReg(
                                           BaseReg, std::move(Offset), AluOp));
...
OperandMatchResultTy FantasyAsmParser::parseOperand(OperandVector *Operands,
                                                    StringRef Mnemonic) {
...
StringRef FantasyAsmParser::splitMnemonic(StringRef Name, SMLoc NameLoc,
                                          OperandVector *Operands) {
  ...
  if (Mnemonic[0] == 'b' ||
    ...
    LPCC::CondCode CondCode =
        LPCC::suffixToFantasyCondCode(Mnemonic.substr(1, Next));
    if (CondCode != LPCC::UNKNOWN) {
      Mnemonic = Mnemonic.slice(0, 1);
      Operands->push_back(FantasyOperand::CreateToken(Mnemonic, NameLoc));
      Operands->push_back(FantasyOperand::createImm(
          MCConstantExpr::create(CondCode, getContext()), NameLoc, NameLoc));
      if (IsBRR) {
        Operands->push_back(FantasyOperand::CreateToken(".r", NameLoc));
      }
      return Mnemonic;
    }
  }
  ...
  if (Mnemonic.startswith("sel") ||
    ...
    LPCC::CondCode CondCode = LPCC::suffixToFantasyCondCode(Mnemonic);
    if (CondCode != LPCC::UNKNOWN) {
...
static bool IsMemoryAssignmentError(const OperandVector &Operands) {
  ...
  if (FantasyOperand *PossibleAluOp =
          static_cast<FantasyOperand *>(Operands[PossibleAluOpIdx].get()))
...
static bool IsRegister(const MCParsedAsmOperand &op) {
  return static_cast<const FantasyOperand &>(op).isReg();
}
...
static bool MaybePredicatedInst(const OperandVector &Operands) {
  ...
  return StringSwitch<bool>(
             static_cast<const FantasyOperand &>(*Operands[0]).getToken())
...
bool FantasyAsmParser::ParseInstruction(ParseInstructionInfo & /*Info*/,
  ...
  if (Lexer.is(AsmToken::EndOfStatement) && Name == "st" &&
    ...
    Operands.insert(Operands.begin(),
                    FantasyOperand::CreateToken("s", NameLoc));
    Operands.insert(Operands.begin() + 1,
                    FantasyOperand::createImm(
  ...
  if (Lexer.is(AsmToken::EndOfStatement) && Name.startswith("bt") &&
    ...
    Operands.insert(Operands.begin(),
                    FantasyOperand::CreateToken("bt", NameLoc));
  }
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeFantasyAsmParser() {
  RegisterMCAsmParser<FantasyAsmParser> x(getTheFantasyTarget());
}
