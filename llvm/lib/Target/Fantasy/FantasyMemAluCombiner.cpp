//===-- FantasyMemAluCombiner.cpp - Pass to combine memory & ALU operations
//-===//
//
...
//
// The Fantasy ISA supports instructions where a load/store modifies the base
...
class FantasyMemAluCombiner : public MachineFunctionPass {
public:
  static char ID;
  explicit FantasyMemAluCombiner() : MachineFunctionPass(ID) {
    initializeFantasyMemAluCombinerPass(*PassRegistry::getPassRegistry());
  }

  StringRef getPassName() const override {
    return "Fantasy load / store optimization pass";
  }
...
char FantasyMemAluCombiner::ID = 0;

INITIALIZE_PASS(FantasyMemAluCombiner, DEBUG_TYPE,
                "Fantasy memory ALU combiner pass", false, false)

namespace {
bool isSpls(uint16_t Opcode) {
  return Fantasy::splsIdempotent(Opcode) == Opcode;
}
...
bool isZeroOperand(const MachineOperand &Op) {
  return ((Op.isReg() && Op.getReg() == Fantasy::R0) ||
...
void FantasyMemAluCombiner::insertMergedInstruction(MachineBasicBlock *BB,
                                                    const MbbIterator &MemInstr,
...
bool isSuitableAluInstr(bool IsSpls, const MbbIterator &AluIter,
  ...
  if (Op2.isImm()) {
    ...
    if (AluIter->getOpcode() != Fantasy::ADD_I_LO)
      return false;

    if (Offset.isReg() && Offset.getReg() == Fantasy::R0)
      return true;
...
MbbIterator FantasyMemAluCombiner::findClosestSuitableAluInstr(
...
bool FantasyMemAluCombiner::combineMemAluInBasicBlock(MachineBasicBlock *BB) {
  bool Modified = false;
...
// machine function
bool FantasyMemAluCombiner::runOnMachineFunction(MachineFunction &MF) {
  if (DisableMemAluCombiner)
    return false;

  TII = MF.getSubtarget<FantasySubtarget>().getInstrInfo();
  bool Modified = false;
...
FunctionPass *llvm::createFantasyMemAluCombinerPass() {
  return new FantasyMemAluCombiner();
}
