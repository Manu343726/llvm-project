//===-- FantasyTargetTransformInfo.h - Fantasy specific TTI ---------*- C++
//-*-===//
//
...
// Fantasy target machine. It uses the target's detailed information to
...
namespace llvm {
class FantasyTTIImpl : public BasicTTIImplBase<FantasyTTIImpl> {
  typedef BasicTTIImplBase<FantasyTTIImpl> BaseT;
  typedef TargetTransformInfo TTI;
  friend BaseT;

  const FantasySubtarget *ST;
  const FantasyTargetLowering *TLI;

  const FantasySubtarget *getST() const { return ST; }
  const FantasyTargetLowering *getTLI() const { return TLI; }

public:
  explicit FantasyTTIImpl(const FantasyTargetMachine *TM, const Function &F)
