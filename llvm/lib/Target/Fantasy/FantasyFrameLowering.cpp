//===-- FantasyFrameLowering.cpp - Fantasy Frame Information
//------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the Fantasy implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
void FantasyFrameLowering::determineFrameLayout(MachineFunction &MF) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  const FantasyRegisterInfo *LRI = STI.getRegisterInfo();
...
// ADJDYNALLOC pseudo instructions with a Fantasy:ADDI with the
// maximum call frame size as the immediate.
void FantasyFrameLowering::replaceAdjDynAllocPseudo(MachineFunction &MF) const {
  const FantasyInstrInfo &LII =
      *static_cast<const FantasyInstrInfo *>(STI.getInstrInfo());
  ...
  for (MachineFunction::iterator MBB = MF.begin(), E = MF.end(); MBB != E;
    ...
    while (MBBI != MBB->end()) {
      ...
      if (MI.getOpcode() == Fantasy::ADJDYNALLOC) {
        DebugLoc DL = MI.getDebugLoc();
        Register Dst = MI.getOperand(0).getReg();
        Register Src = MI.getOperand(1).getReg();

        BuildMI(*MBB, MI, DL, LII.get(Fantasy::ADD_I_LO), Dst)
            .addReg(Src)
...
void FantasyFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  // Push old FP
  // st %fp,-4[*%sp]
  BuildMI(MBB, MBBI, DL, LII.get(Fantasy::SW_RI))
      .addReg(Fantasy::FP)
      .addReg(Fantasy::SP)
      .addImm(-4)
  ...
  // Generate new FP
  // add %sp,8,%fp
  BuildMI(MBB, MBBI, DL, LII.get(Fantasy::ADD_I_LO), Fantasy::FP)
      .addReg(Fantasy::SP)
      .addImm(8)
      .setMIFlag(MachineInstr::FrameSetup);
  ...
  // sub %sp,StackSize,%sp
  if (StackSize != 0) {
    BuildMI(MBB, MBBI, DL, LII.get(Fantasy::SUB_I_LO), Fantasy::SP)
        .addReg(Fantasy::SP)
        .addImm(StackSize)
        .setMIFlag(MachineInstr::FrameSetup);
  }
...
MachineBasicBlock::iterator FantasyFrameLowering::eliminateCallFramePseudoInstr(
...
void FantasyFrameLowering::emitEpilogue(MachineFunction & /*MF*/,
                                        MachineBasicBlock &MBB) const {
  ...
  BuildMI(MBB, MBBI, DL, LII.get(Fantasy::ADD_I_LO), Fantasy::SP)
      .addReg(Fantasy::FP)
      .addImm(0);

  // Restore the frame pointer from the stack.
  BuildMI(MBB, MBBI, DL, LII.get(Fantasy::LDW_RI), Fantasy::FP)
      .addReg(Fantasy::FP)
      .addImm(-8)
      .addImm(LPAC::ADD);
}

void FantasyFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                                BitVector &SavedRegs,
                                                RegScavenger *RS) const {
  TargetFrameLowering::determineCalleeSaves(MF, SavedRegs, RS);

  MachineFrameInfo &MFI = MF.getFrameInfo();
  const FantasyRegisterInfo *LRI =
      static_cast<const FantasyRegisterInfo *>(STI.getRegisterInfo());
  int Offset = -4;
