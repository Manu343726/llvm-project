//===-- FantasyISelLowering.cpp - Fantasy DAG Lowering Implementation
//---------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the FantasyTargetLowering class.
//
//===----------------------------------------------------------------------===//
...
static cl::opt<int> FantasyLowerConstantMulThreshold(
...
FantasyTargetLowering::FantasyTargetLowering(const TargetMachine &TM,
                                             const FantasySubtarget &STI)
    : TargetLowering(TM) {
  // Set up the register classes.
  addRegisterClass(MVT::i32, &Fantasy::GPRRegClass);

  // Compute derived properties from the register classes
  TRI = STI.getRegisterInfo();
  computeRegisterProperties(TRI);

  setStackPointerRegisterToSaveRestore(Fantasy::SP);
...
SDValue FantasyTargetLowering::LowerOperation(SDValue Op,
...
//===----------------------------------------------------------------------===//
//                       Fantasy Inline Assembly Support
//===----------------------------------------------------------------------===//

Register
FantasyTargetLowering::getRegisterByName(const char *RegName, LLT /*VT*/,
  ...
  Register Reg = StringSwitch<unsigned>(RegName)
                     ...
                     .Case("sp", Fantasy::SP)
                     .Case("fp", Fantasy::FP)
                     .Case("rr1", Fantasy::RR1)
                     .Case("r10", Fantasy::R10)
                     .Case("rr2", Fantasy::RR2)
                     .Case("r11", Fantasy::R11)
                     .Case("rca", Fantasy::RCA)
                     .Default(0);
...
FantasyTargetLowering::getRegForInlineAsmConstraint(
    const TargetRegisterInfo *TRI, StringRef Constraint, MVT VT) const {
  if (Constraint.size() == 1)
    // GCC Constraint Letters
    switch (Constraint[0]) {
    case 'r': // GENERAL_REGS
      return std::make_pair(0U, &Fantasy::GPRRegClass);
    default:
      break;
    }
...
FantasyTargetLowering::getSingleConstraintMatchWeight(
...
void FantasyTargetLowering::LowerAsmOperandForConstraint(
...
#include "FantasyGenCallingConv.inc"

static unsigned NumFixedArgs;
static bool CC_Fantasy32_VarArg(unsigned ValNo, MVT ValVT, MVT LocVT,
  ...
  if (ValNo < NumFixedArgs) {
    return CC_Fantasy32(ValNo, ValVT, LocVT, LocInfo, ArgFlags, State);
  }
...
SDValue FantasyTargetLowering::LowerFormalArguments(
...
SDValue
FantasyTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // Fantasy target does not yet support tail call optimization.
  IsTailCall = false;
...
SDValue FantasyTargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &DL,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  ...
  FantasyMachineFunctionInfo *FantasyMFI =
      MF.getInfo<FantasyMachineFunctionInfo>();
  ...
  if (CallConv == CallingConv::Fast) {
    CCInfo.AnalyzeFormalArguments(Ins, CC_Fantasy32_Fast);
  } else {
    CCInfo.AnalyzeFormalArguments(Ins, CC_Fantasy32);
  }
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      case MVT::i32: {
        Register VReg = RegInfo.createVirtualRegister(&Fantasy::GPRRegClass);
  ...
  // The Fantasy ABI for returning structs by value requires that we copy
  // the sret argument into rv for the return. Save the argument into
  // a virtual register so that we can access it from the return points.
  if (MF.getFunction().hasStructRetAttr()) {
    unsigned Reg = FantasyMFI->getSRetReturnReg();
    if (!Reg) {
      Reg = MF.getRegInfo().createVirtualRegister(getRegClassFor(MVT::i32));
      FantasyMFI->setSRetReturnReg(Reg);
    }
  ...
  if (IsVarArg) {
    ...
    FantasyMFI->setVarArgsFrameIndex(FI);
  }
...
SDValue
FantasyTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                   bool IsVarArg,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_Fantasy32);
  ...
  if (DAG.getMachineFunction().getFunction().hasStructRetAttr()) {
    ...
    Chain = DAG.getCopyToReg(Chain, DL, Fantasy::RV, Val, Flag);
    Flag = Chain.getValue(1);
    RetOps.push_back(
        DAG.getRegister(Fantasy::RV, getPointerTy(DAG.getDataLayout())));
  }

  RetOps[0] = Chain; // Update chain

  unsigned Opc = FantasyISD::RET_FLAG;
  if (Flag.getNode())
...
SDValue FantasyTargetLowering::LowerCCCCallTo(
  ...
  if (NumFixedArgs)
    CCInfo.AnalyzeCallOperands(Outs, CC_Fantasy32_VarArg);
  else {
    if (CallConv == CallingConv::Fast)
      CCInfo.AnalyzeCallOperands(Outs, CC_Fantasy32_Fast);
    else
      CCInfo.AnalyzeCallOperands(Outs, CC_Fantasy32);
  }
  ...
  for (unsigned I = 0, J = 0, E = ArgLocs.size(); I != E; ++I) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (StackPtr.getNode() == nullptr)
        StackPtr = DAG.getCopyFromReg(Chain, DL, Fantasy::SP,
  ...
  uint8_t OpFlag = FantasyII::MO_NO_FLAG;
  if (G) {
  ...
  Chain = DAG.getNode(FantasyISD::CALL, DL, NodeTys,
...
SDValue FantasyTargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, RetCC_Fantasy32);
...
static LPCC::CondCode IntCondCCodeToICC(SDValue CC, const SDLoc &DL,
  ...
  // and Fantasy only supports integer comparisons, so only provide definitions
  // for them.
  switch (SetCCOpcode) {
  case ISD::SETEQ:
    return LPCC::ICC_EQ;
...
SDValue FantasyTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue Flag =
      DAG.getNode(FantasyISD::SET_FLAG, DL, MVT::Glue, LHS, RHS, TargetCC);

  return DAG.getNode(FantasyISD::BR_CC, DL, Op.getValueType(), Chain, Dest,
                     TargetCC, Flag);
}

SDValue FantasyTargetLowering::LowerMUL(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (InstrRequired > FantasyLowerConstantMulThreshold)
    return SDValue();
...
SDValue FantasyTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(FantasyISD::SETCC, DL, Op.getValueType(), TargetCC, Flag);
}

SDValue FantasyTargetLowering::LowerSELECT_CC(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
  ...
  return DAG.getNode(FantasyISD::SELECT_CC, DL, VTs, TrueV, FalseV, TargetCC,
                     Flag);
}

SDValue FantasyTargetLowering::LowerVASTART(SDValue Op,
                                            SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  FantasyMachineFunctionInfo *FuncInfo =
      MF.getInfo<FantasyMachineFunctionInfo>();
...
SDValue
FantasyTargetLowering::LowerDYNAMIC_STACKALLOC(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDValue Chain = Op.getOperand(0);
  ...
  // For Fantasy, the outgoing memory arguments area should be on top of the
  ...
  SDValue ArgAdjust = DAG.getNode(FantasyISD::ADJDYNALLOC, DL, MVT::i32, Sub);
...
SDValue FantasyTargetLowering::LowerRETURNADDR(SDValue Op,
                                               SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  MachineFrameInfo &MFI = MF.getFrameInfo();
...
SDValue FantasyTargetLowering::LowerFRAMEADDR(SDValue Op,
                                              SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
  ...
  SDLoc DL(Op);
  SDValue FrameAddr =
      DAG.getCopyFromReg(DAG.getEntryNode(), DL, Fantasy::FP, VT);
  unsigned Depth = cast<ConstantSDNode>(Op.getOperand(0))->getZExtValue();
  while (Depth--) {
...
const char *FantasyTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case FantasyISD::ADJDYNALLOC:
    return "FantasyISD::ADJDYNALLOC";
  case FantasyISD::RET_FLAG:
    return "FantasyISD::RET_FLAG";
  case FantasyISD::CALL:
    return "FantasyISD::CALL";
  case FantasyISD::SELECT_CC:
    return "FantasyISD::SELECT_CC";
  case FantasyISD::SETCC:
    return "FantasyISD::SETCC";
  case FantasyISD::SUBBF:
    return "FantasyISD::SUBBF";
  case FantasyISD::SET_FLAG:
    return "FantasyISD::SET_FLAG";
  case FantasyISD::BR_CC:
    return "FantasyISD::BR_CC";
  case FantasyISD::Wrapper:
    return "FantasyISD::Wrapper";
  case FantasyISD::HI:
    return "FantasyISD::HI";
  case FantasyISD::LO:
    return "FantasyISD::LO";
  case FantasyISD::SMALL:
    return "FantasyISD::SMALL";
  default:
    return nullptr;
  }
}

SDValue FantasyTargetLowering::LowerConstantPool(SDValue Op,
                                                 SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ConstantPoolSDNode *N = cast<ConstantPoolSDNode>(Op);
  const Constant *C = N->getConstVal();
  const FantasyTargetObjectFile *TLOF =
      static_cast<const FantasyTargetObjectFile *>(
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small ||
    ...
    SDValue Small = DAG.getTargetConstantPool(
        C, MVT::i32, N->getAlignment(), N->getOffset(), FantasyII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(Fantasy::R0, MVT::i32),
                       DAG.getNode(FantasyISD::SMALL, DL, MVT::i32, Small));
  } else {
    uint8_t OpFlagHi = FantasyII::MO_ABS_HI;
    uint8_t OpFlagLo = FantasyII::MO_ABS_LO;
    ...
    Hi = DAG.getNode(FantasyISD::HI, DL, MVT::i32, Hi);
    Lo = DAG.getNode(FantasyISD::LO, DL, MVT::i32, Lo);
    SDValue Result = DAG.getNode(ISD::OR, DL, MVT::i32, Hi, Lo);
    return Result;
  }
}

SDValue FantasyTargetLowering::LowerGlobalAddress(SDValue Op,
                                                  SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue FantasyTargetLowering::LowerBlockAddress(SDValue Op,
                                                 SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue FantasyTargetLowering::LowerJumpTable(SDValue Op,
                                              SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small) {
    SDValue Small = DAG.getTargetJumpTable(JT->getIndex(),
                                           getPointerTy(DAG.getDataLayout()),
                                           FantasyII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(Fantasy::R0, MVT::i32),
                       DAG.getNode(FantasyISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue FantasyTargetLowering::LowerSHL_PARTS(SDValue Op,
                                              SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  ...
  // Fantasy shifts of larger than register sizes are wrapped rather than
...
SDValue FantasyTargetLowering::LowerSRL_PARTS(SDValue Op,
                                              SelectionDAG &DAG) const {
  ...
  // Note: This takes advantage of Fantasy's shift behavior to avoid needing to
...
SDValue FantasyTargetLowering::PerformDAGCombine(SDNode *N,
...
void FantasyTargetLowering::computeKnownBitsForTargetNode(
